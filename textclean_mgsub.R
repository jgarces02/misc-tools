...
#' \code{mgsub_regex_safe} this is other arguments passed to 
#' \code{\link[mgsub]{mgsub}}.
...
mgsub <- function (x, pattern, replacement, leadspace = FALSE, 
    ...
    if (!is.null(list(...)$ignore.case) & fixed) {
        warning(
            paste0('`ignore.case = TRUE` can\'t be used with `fixed = TRUE`.\n',
    ...
    if (safe) {
        return(mgsub_regex_safe(x = x, pattern = pattern, 
    ...
    if (any(!nzchar(pattern))) {
        ...
        warning(paste0(
            'Empty pattern found (i.e., `pattern = ""`).\n', 
    ...
    if (trim) {
        x <- gsub("\\s+", " ", gsub("^\\s+|\\s+$", "", x, perl=TRUE), perl=TRUE)
    }
...
mgsub_regex_safe <- function(x, pattern, replacement, ...){
    mgsub(string = x, pattern = pattern, replacement = replacement, ...)
}
